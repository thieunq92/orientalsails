<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="NewPopup.Master" CodeBehind="BookingAddRoom.aspx.cs" Inherits="Portal.Modules.OrientalSails.Web.Admin.BookingAddRoom" %>
        ...
        <div class="row">
            ...
            <div class="col-xs-3">
                ...
                <div class="col-xs-6">
                    ...
                    <asp:Button class="btn btnAddBooking" runat="server" ID="btnAddRoom" Text="Add room" OnClientClick="return addBooking();" />
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        $( document ).ready(function() {
            ...
            $( ".booking-room" ).each(function( index ) {
                $(this).parent().parent().parent().css("background", "#FFEB3B");
            });
        });
        var currentRoomDic = {};
            ...
            if (numberOfDay === 2) {
                ...
                setTimeout(function (){
                    $( ".nextRoom" ).each(function() {
                        let classType = $(this).attr("class-type");
                        let id = $(this).attr("data-id");
                        if (selectRoomIds.indexOf(id)>0) {
                            $(this).prop('checked', true);
                            if(selectNextRoomIds.indexOf(id)<0)selectNextRoomIds.push(id);
                            if (selectNextRooms[classType] > 0) {
                                selectNextRooms[classType] =
                                    selectNextRooms[classType] + 1;
                            } else {
        ...
        function addBooking() {
            ...
            if (isEmpty(selectCurrentRooms)) {
                ...
                if (!isEqual(selectRoomIds.sort(), selectNextRoomIds.sort())) {
                    ...
                    var src = "/Modules/Sails/Admin/BookingAddRoomSelectAdult.aspx?NodeId=1&SectionId=15&roomIds=" + selectRoomIds.toString() + "&bookingId=" + '<%=Request["bookingId"]%>';
